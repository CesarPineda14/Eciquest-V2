...
io.on("connection", (socket) => {
    ...
    socket.on('requestCategories', () => {
        // const categorias = [...new Set(questions.map(q => q.categoria))];

        socket.emit('categories', categorias);
        // // console.log(categorias, "categorias get");
    });
    socket.on('setCategories', (categoria) => {
        categorias = categoria;
        // // console.log(categorias, "categorias set");
    });
    ...
    socket.on('requestQuestions', (categoria) => {
        ...
        if (!salas[roomId]) {
            salas[roomId] = { players: {}, questions: [], currentQuestion: null, correctAnswer: null , pendingResponses: 0 };
        }
        // console.log(salas[roomId])
    ...
    socket.on('resetSorteo', roomId  => {
        if (salas[roomId]) {
            salas[roomId].sorteoActivo = false;
            // console.log('Sorteo reseteado en la sala:', roomId);
        }
    });
    ...
    socket.on('iniciarSorteo', ({ roomId }) => {
        if (salas[roomId] && Object.keys(salas[roomId].players).length > 0) {
            let ganador = Math.random();
            salas[roomId].currentQuestion = null;     
            salas[roomId].correctAnswer = null;
    
            // Imprimir en consola los jugadores de la sala
            // console.log(`Iniciando sorteo para la sala: ${roomId}`);
            Object.keys(salas[roomId].players).forEach(playerId => {
                // console.log(`Enviando sorteo a jugador: ${playerId}`);
            });
    ...
    socket.on("answer", ({ roomId, answer }) => {
        ...
        let player = salas[roomId].players[socket.id];
        
        // console.log(order, "order")
        if (isCorrect) {
            let order = salas[roomId].responseOrder++;
            if (order === 0) {
                player.score += 20; 
            } else if (order === 1) {
                player.score += 15; 
            ...
            // console.log(salas[roomId].players, "salas")
        }
        ...
        console.log(salas[roomId].pendingResponses, "pendingResponses")
